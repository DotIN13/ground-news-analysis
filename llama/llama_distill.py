...
class TextClassificationDataset(Dataset):
    ...
    def __getitem__(self, idx):
        text = f"{self.data.iloc[idx]['article_description']} {self.data.iloc[idx]['article_text']}"
...
def load_datasets(df_path, label_map, tokenizer):
    ...
    train_dataset = TextClassificationDataset(train_df, label_map, tokenizer)
...
def setup_model_and_tokenizer(model_name, bnb_config, accelerator, lora_weights_path=None):
    ...
    if lora_weights_path is not None:
        ...
        model.load_adapter(lora_weights_path, adapter_name="lora", is_trainable=True)
...
def main():
    ...
    # lora_weights_path = "best_model/best_model_r16_b4"
    ...
    train_dataloader, val_dataloader = load_datasets(
        "../data/topics_10k_deepseek_5_class.csv", label_map, tokenizer)
    ...
    writer = SummaryWriter(log_dir="./runs/exp-distill-5-class")
